...
config["tumor_type"] = [i['class_tumor'] for i in tumor_zoo_params_dict_list if i['class_tumor'] is not 'HEALTHY']
...
config["all_modalities"] = ["T1c_subtrMeanDivStd", "OTMultiClass"]
...
def set_fold(fold, exp):
    ...
    # Read excel
    df = pd.read_excel(config['excel_path'], sheet_name="internal_MENING")
