...
from brats_nclass_testing_half.train_isensee2017 import trim_df_by_dropping_nans, trim_df_based_on_GT, filter_sessions_based_on_availability_in_scratch, \
    ...
    split_data_into_n_folds_from_excel, trim_df_based_on_presence_in_scratch_and_modality
...
def set_fold(fold, exp):
    ...
    # Read excel
    df = pd.read_excel(config['excel_path'], sheet_name="Sheet1_MENING")
    ...
    df = df[df.fold.str.contains("fold")]
    # End
    ...
    config['training_sessions'], config['validation_sessions'], config['testing_sessions'] = split_data_into_n_folds_from_excel(config, info, df, sessions_abspath_exists)
