...
def visualize_cam_init_optimizer(model, layer_idx, filter_indices, penultimate_layer_idx=None, backprop_modifier=None):
    
    print("Initialized opt")
    ...
    opt = Optimizer(model.input, losses, wrt_tensor=penultimate_layer.output, norm_grads=False)

    return opt

def visualize_cam_with_preinitialized_optimizer(input_tensor, seed_input, opt, grad_modifier=None):
    """Generates a gradient based class activation map (CAM) by using positive gradients of `input_tensor`
    with respect to weighted `losses`.
