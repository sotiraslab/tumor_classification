...
# Using the condition "if len(np.unique(data[:,:,i]))>1" to remove slices with just background and because background value is not 0
# slices = [data[:,:,i] for i in range(0,num_slices_cor,5) if len(np.unique(data[:,:,i]))>1]
slices = [data[:,i,:] for i in range(0,num_slices_cor,5)]
...
for j in range(num_rows):
    slices = [T1c_fm_concat[:,:,:,j][:,m,:] for m in range(0,num_slices_cor,5)]
    for i, slice in enumerate(slices):
        if j == 0:
            colmap = "gray"
        else:
