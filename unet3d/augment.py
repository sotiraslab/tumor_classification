...
def simulate_partial_acq(data, truth, simulate_partial_acq_prob):
    GT = truth
    partial_mask = np.squeeze(nib.load("/home/satrajit/tumor_classification_revamped/partial_brainmask_128x128x128.nii.gz").get_fdata())
    # partial_mask = np.squeeze(nib.load("/tumor_classification_revamped/partial_brainmask_128x128x128.nii.gz").get_fdata())
    lesion_inside = np.count_nonzero(partial_mask) == np.count_nonzero(partial_mask + GT)

    if lesion_inside:
        if random.uniform(0, 1) < simulate_partial_acq_prob:
            print("[DEBUG] simulate_partial_acq TRUE ---------------------------------------------------------------")
            t1c_partial = data * partial_mask
            gt_partial = GT * partial_mask

            return t1c_partial, gt_partial
        else:
            print("[DEBUG] simulate_partial_acq FALSE because (random > threshold) @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")

            return data, truth
    else:
        print("[DEBUG] simulate_partial_acq FALSE because (lesion outside) xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        return data, truth

def shift_scale_intensity(image, prms):

    n_channs = 1

    print("[INFO] shift_scale_intensity performed on data")
    ...
    image_shift_scaled = np.copy(image.get_data())
    image_shift_scaled = (np.expand_dims(image_shift_scaled,axis=0) + shift_per_chan ) * scale_per_chan
    
    return new_img_like(image, data=np.squeeze(image_shift_scaled))
...
def distort_image(image, flip_axis=None, scale_factor=None, intensity_distortion = None):
    # print("[DEBUG] intensity_distortion is",intensity_distortion)
    if flip_axis:
        image = flip_image(image, flip_axis)
    if scale_factor is not None:
        image = scale_image(image, scale_factor)
    if intensity_distortion is not None:
        # print("[DEUBG] Doing intensity_distortion")
        image = shift_scale_intensity(image,intensity_distortion)
    return image


def augment_data(data, truth, affine, scale_deviation=None, flip=[0,1,2], intensity_distortion = None):
    # print("[DEBUG] Augmentation performed >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    # truth = (144,144,144), n_dim = 3
    ...
    if flip:
        ...
        # print("[DEBUG] flip axis >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", flip_axis)
    else:
        flip_axis = None
    ...
    for data_index in range(data.shape[0]):
        ...
        data_list.append(resample_to_img(distort_image(image, flip_axis=flip_axis, scale_factor=scale_factor, intensity_distortion=intensity_distortion), image, interpolation="continuous").get_data())

    data = np.asarray(data_list)
    truth_image = get_image(truth, affine)
    truth_data = resample_to_img(distort_image(truth_image, flip_axis=flip_axis, scale_factor=scale_factor), truth_image, interpolation="nearest").get_data()
